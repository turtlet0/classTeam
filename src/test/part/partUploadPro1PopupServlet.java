...
@MultipartConfig(
		...
		location = "D:\\workspace_mocaclass\\mocaclass\\WebContent\\partUpload" // 업로드된 파일이 저장될 디렉토리를 String 타입으로 지정
		)
public class partUploadPro1PopupServlet extends HttpServlet { // HttpServlet 상속
	 ...
	 */
	public partUploadPro1PopupServlet (){
		super();
	}
	...
	protected void doPost(HttpServletRequest request, 
		...
		Part part = request.getPart("partFile1"); // javax.servlet.http.Part;
		...
		FileDTO fdto = null;
		if(session.getAttribute("fdto") == null) {
			System.out.println("fdto 새로 생성");
			fdto = new FileDTO();
		} else {
			System.out.println("cdto 기존");
			fdto = (FileDTO) session.getAttribute("fdto");
		}
		
		// fdto에 정보 저장 
		fdto.setUploadFileName(uploadFileName);
