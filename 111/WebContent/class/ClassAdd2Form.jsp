...
<script src="${pageContext.request.contextPath}/js/classResultImgUpload.js"></script> <!-- 클래스 결과 이미지 업로드 js -->
<%
...
<script type="text/javascript" src="/LatteClass_merge/se2/js/HuskyEZCreator.js" charset="utf-8"></script>
...
<script type="text/javascript">
	...
	$(function(){
		...
		nhn.husky.EZCreator.createInIFrame({
			...
			sSkinURI: "/LatteClass_merge/se2/SmartEditor2Skin.html",  
			htParams : {
...
$(document).ready(function(){
	...
	$('#classAddForm').on('input', function (){ // form 태그 안에서 input 발생한 경우
		/* if($('#free_curriculum').val() == "" && ($('#duration_time').val() == "" || $('#step1').val() == "") ){
			
		} */
		if($('#free_curriculum').val() == ""){
			$('#nextSubmit').attr('disabled', true);
		}else {
			...
			<div>
				...
				<div>
					...
					<div name="freeCurriculum">
						<textarea rows="10" id="free_curriculum" name="free_curriculum" placeholder="커리큘럼(클래스 진행 순서)를 적어주세요.">${sessionScope.cdto.free_curriculum }</textarea>
					</div>
				...
				<div>
					...
					<div>
						<input type="hidden" id="classResult_img_idx" name="img_idx" value="classResult">
						<button type="button" name="classResultImgUploadBtn">
							<span>이미지 업로드</span><br>
							<span>jpg, jpeg, jfif, png, bmp 확장자만 업로드 가능합니다.</span><br>
							<span>5MB 이하 용량의 이미지만 업로드 가능합니다.</span>
							
							<input type="file" id="classResult_img_file" accept="image/*" hidden="hidden">
						</button>
							
						<div id=classResultImgResult>
						<!-- JSTL foreach : sessionScope.classResultFdtoArr -->
						</div>
					</div>
				</div>	
				<br>
				<div>
					<p>4) 이런분들이 들으면 좋아요</p>
					<textarea rows="3" id="recommend_people" name="recommend_people" 
						placeholder="셀프인테리어를 위한 소품이 필요하신 분, 연인 선물이 필요하신 분" style="height: 18px;">${sessionScope.cdto.recommend_people }</textarea>
				</div>
				<br>
				<%-- <div>
				...
				<div>
					...
					<input type="submit" value="다음" id="nextSubmit" onclick='classAddSubmit(3)'>
				</div>
			</div>
		</form>
	</div>
