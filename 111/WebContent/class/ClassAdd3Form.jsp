...
$(document).ready(function(){
	alert('ClassAdd3Form - jquery  정상 동작');
	
	// opning_area
	var opening_area = '${sessionScope.cdto.opening_area}';
	if(opening_area != null && opening_area != ""){
		//alert(opening_area);
		$('input:radio[name=opening_area]:radio[value='+opening_area+']').attr('checked', true);
	}
	
	// convenience_option 제어
	var convenience_option = '${sessionScope.cdto.convenience_option}';
	var convenience_optionArr = convenience_option.split(",");
	if(convenience_option != null && convenience_option != ""){
		for(var i=0; i<convenience_optionArr.length; i++){
			$('input:checkbox[name=convenience_option]:checkbox[value='+convenience_optionArr[i]+']').attr('checked', true);
		}
	}
	
	// health_safety_option 제어
	var health_safety_option = '${sessionScope.cdto.health_safety_option}';
	var health_safety_optionArr = health_safety_option.split(",");
	if(health_safety_option != null && health_safety_option != ""){
		for(var i=0; i<health_safety_optionArr.length; i++){
			$('input:checkbox[name=health_safety_option]:checkbox[value='+health_safety_optionArr[i]+']').attr('checked', true);
		}
	}
		
	/* 유효성 체크 */
	$('#finalSubmit').attr('disabled', true);
	$('#classAddForm').on('input', function (){
		if($('input[name=opening_area]:checked').val() == undefined){
			$('#finalSubmit').attr('disabled', true);
		} else{
...
%>
	...
	<div>
		...
		<hr>
		<form id="classAddForm" method="post">
			...
			<div>
				...
				<div>
					...
					<input type="text" id="roadAddress" name="road_address" placeholder="도로명주소"  
						readonly="readonly" value="${sessionScope.cdto.road_address }" >
					<input type="text" id="jibunAddress" name="jibun_address" placeholder="지번주소" 
					readonly="readonly" value="${sessionScope.cdto.jibun_address }">
			...
			<div>
				<p>10. 찾아오는 길 추가 설명</p>
				<input type="text" id="directions" name="directions" value="${sessionScope.cdto.directions }">
			...
			<div>
				...
				<input type="submit" value="최종 등록" id="finalSubmit" onclick='classAddSubmit(4)'>
			</div>
		</form>
	</div>
	...
	<script>
		function classAddSubmit(idx){
			if(idx == 1) {
				...
				classAddForm.action = './ClassAdd.cl?idx=4';
				classAddForm.submit();
			}
		}
	</script>
