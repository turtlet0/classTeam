...
$(document).ready(function(){
	...
	/* Ajax 이용 이미지 파일 전송 */
	alert(document.querySelector('#classRepReturnImg'));
	/* formData에 file 담기 */
	var img_file = document.querySelector('#classRep_img_file'); // document.querySelector(selector) : 첫번째 요소 반환
	//var img_idx = document.querySelector('#img_idx'); // 서버단에서 전달받을때 [object HTMLInputElement] 가 반환되어버림..
	var img_idx = document.getElementById('classRep_img_idx').value; // 문자열이나 객체가 아닌 실제값(actual value)을 가져와야함
	
	// button 태그 클릭 시 input file 업로드 창 열기
	var imgUploadBtn = document.querySelector('#classRepImgUploadBtn');
	...
	img_file.addEventListener('change', function() {
		...
		// 기존 이미지 제거
		var classRepReturnImgSrc = $('#classRepReturnImg').attr('src');
		if(classRepReturnImgSrc != "/LatteClass_merge/upload//") {
		/*if(document.querySelector('#returnImg') != null) {*/
			alert("기존 이미지 제거");
			var imgUploadPath = $('#classRepReturnImg').attr('src');
			alert($('#classRepReturnImg').attr('src'));
			
			$.ajax({
				url: 'FileDeleteAction.fi',
				data: {imgUploadPath : imgUploadPath}, // 하나의 데이터라도 {파라메터명 : 변수} 꼴로 해야 에러안남
				success: function(){
					alert("삭제 성공!");
					$('#classRepImgResult').empty();
				}
			});
		...
		var imgFileValue = $('#classRep_img_file').val();
		...
		$.ajax({
		    ...
		    success : function (data) {
		    	...
		    	alert(data);
		    	$('#classRepImgResult').empty(); // 선택된 요소의 자식 요소 전체 삭제
		    	$('#classRepImgResult').append(data);
			}
		});
	}); /* Ajax 이용 이미지 파일 전송 */
