...
public class FileDAO {
	...
	// class_cd는 차후 클래스 최초 등록 메서드에서 DB 저장
	...
	} // insertImg(FileDTO fdto)
	
	/*이미지 파일 제거 메서드*/
	public void deleteImg(FileDTO fdto) {
		try {
			con = getCon();
			sql = "DELETE FROM class_img WHERE member_cd = ? AND img_upload_path = ? AND img_name =?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, fdto.getMember_cd());
			pstmt.setString(2, fdto.getImg_upload_path());
			pstmt.setString(3, fdto.getImg_name());
