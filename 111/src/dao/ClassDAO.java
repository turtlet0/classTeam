...
public class ClassDAO {
	...
	// 클래스 세팅
	public List getMyClassList(String cd){
		List myClassList = new ArrayList();
		
		try {
			con = getCon();
			
			sql = "select c.class_name, c.class_cd from class c inner join tutor t "
					+ "on c.class_tutor_cd = t.tutor_cd "
					+ "inner join member m "
			...
			while(rs.next()){
				...
				cdto.setClass_cd(rs.getString("class_cd"));
				cdto.setClass_name(rs.getString("class_name"));
				
				myClassList.add(cdto);
